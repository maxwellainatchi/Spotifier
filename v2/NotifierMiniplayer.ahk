...
CheckWindow: ;checks to see if the window's title has changed
	IniRead, tempPlaying, output.ini, Flags, playing
	IniRead, tempTitle, output.ini, TrackInfo, name
	IniRead, tempArtist, output.ini, TrackInfo, artist
	IniRead, tempAlbum, output.ini, TrackInfo, album
	if (tempTitle != Title || tempArtist != Artist || tempAlbum != Album || tempPlaying!= Playing) {
		Playing := tempPlaying ;Set the new playing
		Title := tempTitle ;Set the new title
		Artist := tempArtist ;Set the new artist
		Album := tempAlbum ;Set the new album
...
CreateGui: ;create the gui and set the text
	...
	TextBody = %TextBody%Title: %Title% `r`nArtist: %Artist% `r`nAlbum: %Album%

	Gui, add, Picture, w20 h20 xp+0 yp+10 gLastSong, %PreviousSource% ;add the previous button
	if (Playing != "false") { ;if a song is playing
